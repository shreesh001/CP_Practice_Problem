...
void solve() {
    int n;
    long long x; // Use long long to avoid overflow during division
    cin >> n >> x;
    ...
    // Condition 1: At least max(a_i) customers
    // Condition 2: At least ceil(sum / x) customers
    // Ceiling division of A/B is (A + B - 1) / B
    long long ans = max(max_a, (sum_a + x - 1) / x);

...
void solve() {
    ...
    prefE.push_back(lvlnode[0]);
    prefO.push_back(lvlnode[1]);
    for (int i=2;i<lvlnode.size();i++){
        if (i%2==0){
            prefE.push_back(lvlnode[i]+prefE.back());
        }else{
            prefO.push_back(lvlnode[i]+prefO.back());
        }
    }
    ll ans=0;
    for (int i=3;i<lvlnode.size();i++){
        if (i%2==1){
            int ind=(i-1)/2-1;
            ans+=lvlnode[i]*prefE[ind];
        }else{
            int ind=(i-1)/2-1;
            ans+=lvlnode[i]*prefO[ind];
        }
    }
    cout<<ans<<endl;
}

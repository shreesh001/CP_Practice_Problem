...
void solve() {
    ...
    ll minidiff=1e18;
    for (int i=1;i<n;i++){
        minidiff=min(minidiff,a[i]-a[i-1]);
    }
    ...
    for (int i=0;i<n;i++){
        for (int j=i+1;j<n;j++){
            ll d=abs(a[i]-a[j]);
            minidiff2=min(minidiff2,d);
            auto it=lower_bound(a.begin(),a.end(),d);
            if (it==a.begin()){
                minidiff2=min(minidiff2,abs(d-a[0]));
            }
            else if (it==a.end()){
                minidiff2=min(minidiff2,abs(d-a[n-1]));
            }
            else{
                minidiff2=min(minidiff2,abs(d-a[it-a.begin()]));
                minidiff2=min(minidiff2,abs(d-a[(it-a.begin())-1]));
            }
        }
    }

...
// ---------------- SOLVE FUNCTION ----------------
ll dp[20][2][11][2];
ll solve(string &s,int ind,int tight,int prev,int lz){
    if (ind==s.size()) return 1;

    if (dp[ind][tight][prev][lz]!=-1) return dp[ind][tight][prev][lz];
    ll res=0;
    ll lb=0;
    ll ub=(tight==1)? s[ind]-'0':9;
    for (int dig=lb;dig<=ub;dig++){
        if (prev==dig && !lz) continue;
        res+=solve(s,ind+1,(tight==1 && dig==ub),dig,(lz && dig==0));
    }
    return dp[ind][tight][prev][lz]=res;
}
void solve() {
    ll a,b;
    cin>>a>>b;
    ...
    ll high=solve(r,0,1,10,1);
    memset(dp,-1,sizeof(dp));
    ll lower=solve(l,0,1,10,1);
    ll ans=high-lower;
    cout<<ans<<"\n";
}

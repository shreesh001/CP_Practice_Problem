...
void solve() {
    ll n,k;
    cin>>n>>k;
    vector<ll>nums(n+1,0),prefor(n+1,0),suffor(n+2,0);
    ll x,a,b,c;
    cin>>x>>a>>b>>c;
    nums[1]=x;
    for (int i=2;i<=n;i++) {
    ...
    for (int i=1;i<=n;i++){
        if ((i-1)%k==0) prefor[i]=nums[i];
        else prefor[i]=prefor[i-1]|nums[i];
    }

    for (int i=n;i>=1;i--){
        if (i%k==0) suffor[i]=nums[i];
        else suffor[i]=suffor[i+1]|nums[i];
    }

    ll ans=0;
    for (int i=k;i<=n;i++){
        if (i%k==0) ans=ans^prefor[i];
        else ans=ans^(prefor[i]|suffor[i-k+1]);
    }
    cout<<ans<<"\n";
